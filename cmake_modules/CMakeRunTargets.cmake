...
message("Using AlloProject Run facilties.")
...
if(BUILDING_cuttlebone)
    get_target_property(CUTTLEBONE_LIBRARY cuttlebone LOCATION)
    add_dependencies(${APP_NAME} cuttlebone)
...
# TODO better copying of resources to build directory
#file(GLOB PROJECT_RES_FILES "${CMAKE_CURRENT_SOURCE_DIR}/${SOURCE_DIR}/*")
#message("Copying resources: ${PROJECT_RES_FILES} from ${CMAKE_CURRENT_SOURCE_DIR}/${SOURCE_DIR}")
#file(COPY ${PROJECT_RES_FILES} DESTINATION ${EXECUTABLE_OUTPUT_PATH})
...
add_custom_target("${APP_NAME}_run"
  COMMAND "${ALLOPROJECT_DEBUGGER}" "-ex" "run" "${EXECUTABLE_OUTPUT_PATH}/${APP_NAME}"
  DEPENDS "${APP_NAME}"
  WORKING_DIRECTORY "${CMAKE_SOURCE_DIR}"
  SOURCES ${ALLOPROJECT_APP_SRC}
  COMMENT "Running: ${APP_NAME}")
